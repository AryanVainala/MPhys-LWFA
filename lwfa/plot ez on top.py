...
def update_plot(iteration):
    ...
    E_plasma, info_Ez = ts.get_field(field='E', coord='z', iteration=iteration, m=0)
    E_plasma_line = E_plasma[0, :]
    
    # Get laser field (mode 1 - linearly polarized)
    E_laser, _ = ts.get_field(field='E', coord='z', iteration=iteration, m=1)
    E_laser_line = E_laser[0, :]
    ...
    # Plot E_z components
    ax.ax2 = ax.twinx()
    ax.ax2.plot(z_E, E_laser_line, 'red', linewidth=1.5, label='Laser (m=1)', alpha=0.7)
    ax.ax2.plot(z_E, E_plasma_line, 'orange', linewidth=2.0, label='Plasma Wake (m=0)')
